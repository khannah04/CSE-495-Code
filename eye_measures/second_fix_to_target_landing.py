...
Calculate time from second fixation onset to first landing on target and correlate with image quality metrics.
Uses precomputed metrics from plausible_realistic prompt experiments.
Eye metric: T[first_target_landing_index] - T[1] (search time after initial orientation)
"""
...
def load_fixation_data():
    """Load fixation data and extract second fix to target landing times."""
    ...
    for trial in data:
        ...
        # Need at least 2 fixations
        if len(T) < 2 or len(bbox) != 4:
            continue
        
        # Find first target landing
        ...
        if first_target_index is None or first_target_index < 1:
            continue
        
        # Calculate time from second fixation to first landing on target
        # T contains durations, so sum from T[1] to T[first_target_index-1]
        search_time = sum(T[1:first_target_index])
        
        image_name = trial['name']
        task = trial['task']
        eye_data[task][image_name].append(search_time)
        ...
        for image_name in eye_data[category].keys():
            ...
            for eye_val in eye_values:
                row = {
                    ...
                    'second_fix_to_target_time': eye_val
                }
...
def compute_correlations(df):
    """Compute correlations between second fix to target time and image metrics."""
    correlations = {}
    ...
    for metric in IMAGE_METRICS:
        ...
        valid_mask = df['second_fix_to_target_time'].notna() & df[metric].notna()
        if valid_mask.sum() < 3:
            continue
        
        try:
            r, p = pearsonr(df.loc[valid_mask, 'second_fix_to_target_time'], 
...
def compute_correlations_by_category_with_aggregation(df, stat_func='mean'):
    ...
    Args:
        df: DataFrame with columns: category, image_name, second_fix_to_target_time, and image metrics
    ...
    for category in categories:
        ...
        if stat_func == 'mean':
            agg_eye = cat_df.groupby('image_name')['second_fix_to_target_time'].mean()
        elif stat_func == 'median':
            agg_eye = cat_df.groupby('image_name')['second_fix_to_target_time'].median()
        elif stat_func == 'min':
            agg_eye = cat_df.groupby('image_name')['second_fix_to_target_time'].min()
        elif stat_func == 'max':
            agg_eye = cat_df.groupby('image_name')['second_fix_to_target_time'].max()
        elif stat_func == 'std':
            agg_eye = cat_df.groupby('image_name')['second_fix_to_target_time'].std()
        else:
            continue
...
def compute_correlations_by_category(df):
    ...
    for category in categories:
        ...
        for metric in IMAGE_METRICS:
            ...
            valid_mask = cat_df['second_fix_to_target_time'].notna() & cat_df[metric].notna()
            if valid_mask.sum() < 3:
                continue
            
            try:
                r, p = pearsonr(cat_df.loc[valid_mask, 'second_fix_to_target_time'],
...
def compute_statistics_by_category(df):
    ...
    categories = sorted(df['category'].unique())
    all_metrics = ['second_fix_to_target_time'] + IMAGE_METRICS
...
def correlate_statistics_across_categories(stats_dfs, eye_metric_name='second_fix_to_target_time'):
...
def main():
    print("=" * 60)
    print("Second Fixation to Target Landing Time Analysis")
    ...
    for blur_type in BLUR_TYPES:
        ...
        excel_file = output_dir / "second_fix_to_target_landing_correlations_by_statistic.xlsx"
        ...
        with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
            for stat_name in ['mean', 'median', 'min', 'max', 'std']:
                ...
                stat_corr.loc['pooled_r'] = compute_overall_correlation(matched_df, 'second_fix_to_target_time', stat_name)
                stat_corr.to_excel(writer, sheet_name=stat_name)

            raw_tab = matched_df[['category', 'image_name', 'second_fix_to_target_time']].copy()
            raw_tab = raw_tab.rename(columns={'second_fix_to_target_time': 'second_fix_to_target'})
